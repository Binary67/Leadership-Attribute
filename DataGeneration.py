...
def GenerateDummyData(DictLeadershipAttribute):
    ...
    if len(DictLeadershipAttribute) != 27:
        print(f"Warning: Expected 27 leadership attributes, but got {len(DictLeadershipAttribute)}")
    ...
    for EachFeedback in AllFeedbacks:
        for AttributeName, AttributeDefinition in DictLeadershipAttribute.items():
            rows.append({
